...
| Services hosted in another containers network need to have ports exposed from the other container. add a label to the port attribution to list the port as a child of the service the port belongs to, as well as update the label to not just the port, but the name of the service the port actually belongs to ||
| Support extracting hostnames with HostRegexp, where one or more top level conditions is Host() ||
